...
public class TraceManager_WAP extends TraceManager {
	...
	public int recordEvent(List<ScanResult> scanResults, int scanTimeOffsetMillis) {
		...
		Log.d("gps", "*wap-event* => +"+(scanTimeOffsetMillis)+"ms");
		...
		// time offset (centi-seconds) of event relative to start time: 2 bytes  
		bytes.append_2(encodeCentiseconds(scanTimeOffsetMillis));
