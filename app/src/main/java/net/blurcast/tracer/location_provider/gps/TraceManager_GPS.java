...
public class TraceManager_GPS extends TraceManager {
	...
	public int recordEvent(Location location) {
		Log.w(TAG, "recordEvent(); locationTime: "+location.getTime()+"; currentTime: "+System.currentTimeMillis());
		
		Log.d("gps", "*gps-event* => +"+(location.getTime()-sensorLocationProviderTimeStart)+"ms");
		...
		if(writeToTraceFile(byteBuilder.getBytes())) {
			Log.w(TAG, "bytes written to file");
