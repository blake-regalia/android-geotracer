...
import net.blurcast.tracer.location_provider.LocationHelper;
import net.blurcast.tracer.location_provider.LocationProviderDefinition;
import net.blurcast.tracer.location_provider.LocationResultsCallback;
...
public class SensorLocationProvider_GPS extends SensorLocationProvider implements LocationResultsCallback {
	...
	public SensorLocationProvider_GPS(SensorLooperCallback callback, Context context, Looper looper, LocationProviderDefinition definition) {
	...
	public synchronized void locationReady(Location location) {
		...
		case MODE_FIRST_LOCATION:
			...
			Log.d("gps", "} location ready: @"+location.getTime()+" <now> "+(System.currentTimeMillis()-location.getTime())+"ms");
			Log.d("gps", "} time started: "+sensorLocationProviderTimeStarted+"  <1st fix time after start> "+(location.getTime()-sensorLocationProviderTimeStarted)+"ms");
